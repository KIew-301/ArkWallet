...
internal static class OverviewReporter
{
    ...
    private static string Build(DateTime generatedAt, RunReport current, IReadOnlyList<RunReport> baselineRuns, string? baselineLabel)
    {
        ...
        sb.AppendLine("<footer>База сравнения: последний прогон в <code>Reports/archive</code>, где есть сценарий, либо выбранный целевой прогон (переменная <code>ARKWALLET_PERF_TARGET</code>, сохраняется в <code>Reports/target.txt</code> до смены). Сценарии без замера в базе помечаются «Нет данных». Статус: «Улучшено» при Δ запросов ≤ −2% или Δ строк ≤ −2%, либо когда время было &gt;20 мс и снизилось на ≥60%; «Регресс» при Δ строк ≥ +2% или (Δ запросов ≥ +10% при количестве ≥10); если сработали оба условия — «Стабильно». Время подсвечивается цветом справочно (для сценариев ≥10 мс) и влияет на статус только через правило «Улучшено». Repeat-прогон (медиана из N замеров): <code>ARKWALLET_PERF_REPEAT=100 dotnet test ArkWallet.PerformanceTests --filter \"FullyQualifiedName~Repeats\"</code>.</footer>");
        sb.AppendLine("</body>");
        sb.AppendLine("</html>");
        return sb.ToString();
    }
