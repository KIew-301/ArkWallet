        ...
        return await ServiceErrorHandler.ExecuteAsync(async () =>
        {
            return await TransactionHandler.ExecuteAsync(dbContext, async () =>
            {
                ...
                var user = User.Load(
                    ...
                    cache.Portfolio.Select(p => Mapper.ToTokens(p.Item, p.Price)).ToList(),
                    cache.SentGifts);
                ...
                foreach (var record in cache.Portfolio.Select(entry => entry.Item))
                {
                    if (tokensBySymbol.TryGetValue(record.CharacterTokenId, out var token))
                        Mapper.ApplyToRecord(record, token);
                    else
