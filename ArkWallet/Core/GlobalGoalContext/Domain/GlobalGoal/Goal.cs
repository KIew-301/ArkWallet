...
namespace ArkWallet.Core.GlobalGoalContext.Domain.Goal;

internal class Goal : AggregateRoot
{
    ...
    private Goal(long id, string name, string description, decimal target, decimal actual)
    {
        Id = id;
        Name = name;
        ...
        string symbolForReward, decimal amountForReward)
    {
        ...
        var goal = new Goal(id, normalizedName, normalizedDescription, target, 0m);
        goal._steps.Add(new GlobalGoalStep(1, target, symbolForReward?.Trim() ?? string.Empty, amountForReward));
        return goal;
    }

    internal static Goal Load(GlobalGoalAggregateData data)
    {
        var goal = new Goal(data.Id, data.Name, data.Description, data.Target, data.Actual);
...
/// <summary>
/// Слепок состояния агрегата Goal для гидратации из БД.
